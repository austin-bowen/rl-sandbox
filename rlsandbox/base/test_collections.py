...
class TestCircularList(unittest.TestCase):
    def setUp(self):
        self.list = CircularList(max_size=3)

    def test_init(self):
        self.assertListEqual([], CircularList())
        self.assertListEqual([], CircularList(max_size=3))
        self.assertListEqual([0, 1, 2], CircularList(range(3), max_size=3))
        self.assertListEqual([6, 4, 5], CircularList(range(7), max_size=3))

    def test_append(self):
        l = self.list

        self.assertEqual(0, len(l))
        self.assertFalse(l.is_max_size)
